...
{
    constexpr Matrix3X3<T>(T xx, T xy, T yx, T yy)
        : xx{xx}, xy{xy}, xz{0}, yx{yx}, yy{yy}, yz{0}, zx{0}, zy{0}, zz{1}
    {
    }
    ...
    {
        return Matrix3X3<T>(-xx, -xy, -yx, -yy);
    };
    template <typename Phi>
    constexpr static Matrix3X3<T> fromRotation(Phi rotation)
    {
        return Matrix3X3<T>(std::cos(rotation), -std::sin(rotation),
                            std::sin(rotation), std::cos(rotation));
    };
    constexpr static Matrix3X3<T> fromTranslation(T&& x, T&& y)
    {
    ...
    constexpr Matrix3X3<T> reverseTranslation()
    {
        return Matrix3X3(xx, xy, -xz, yx, yy, -yz, zx, zy, zz);
    }
    constexpr Matrix3X3<T> reverseRotation()
    {
        return Matrix3X3(xx, -xy, xz, -yx, yy, -yz, zx, zy, zz);
    }
};
template <typename T>
constexpr Matrix3X3<T> operator*(const Matrix3X3<T>& left,
