...
struct Vector2D
{
    ...
    [[nodiscard]] constexpr auto operator-() const { return Vector2D<V>(-x, -y); };
    [[nodiscard]] constexpr auto length() const { return std::hypot(x, y); }
    ...
    constexpr auto normalized() const { return Vector2D<V>(x / length(), y / length()); }
    constexpr auto operator+(V right) const { return Vector2D<V>(x + right, y + right); }
    template <typename T>
    constexpr auto operator/(T divisor)
    {
...
struct Position
{
    P x;
    P y;
    constexpr auto operator-() const { return Position<P>(-x, -y); };
...
[[nodiscard]] constexpr auto operator*(const Matrix3X3<M>& mat, const Vector2D<V>& vec)
{
...
[[nodiscard]] constexpr auto operator*(const Vector2D<V>& vec, const S& scalar)
{
    return Vector2D<V>(vec.x * scalar, vec.y * scalar);
};
template <std::floating_point V, arithmetic S>
[[nodiscard]] constexpr auto operator*(const S& scalar, const Vector2D<V>& vec)
{
    return Vector2D<V>(vec.x * scalar, vec.y * scalar);
};
...
template <typename P>
[[nodiscard]] constexpr auto operator+(const Position<P>& left, const Position<P>& right)
{
    return Position<P>(left.x + right.x, left.y + right.y);
}
template <std::signed_integral P, std::floating_point V>
[[nodiscard]] constexpr auto operator+(const Position<P>& pos, const Vector2D<V>& vec)
{
...
[[nodiscard]] constexpr auto operator-(const Position<P>& pos1, const Position<P>& pos2) -> Vector2D<double>
{
    return Vector2D(static_cast<double>(pos2.x - pos1.x), static_cast<double>(pos2.y - pos1.y));
}
template <std::floating_point P>
[[nodiscard]] constexpr auto operator-(const Position<P>& pos1, const Position<P>& pos2) -> Vector2D<P>
{
...
[[nodiscard]] constexpr auto operator*(const Matrix3X3<M>& left, Position<P> right) -> Position<P>
{
