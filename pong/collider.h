...
class Collider
{
  public:
    ...
    virtual auto getHitbox() const -> const Rectangle<T>& = 0;
    virtual void collide(Vector2D<double>) = 0;
    template <size_t S1, size_t S2>
    constexpr static auto overlap(const ConvexPolygon<T, S1>& polygon1, const ConvexPolygon<T, S2>& polygon2,
                                  const Vector2D<double> velocity) -> std::pair<bool, Vector2D<double>>
    {
        ...
        {
            auto [overlap, mtv] = shadowsOverlap<S1, S2>(normal, vertices1, vertices2);
        ...
        if (Vector2D<double>(polygon2.center() - polygon1.center()) * minimumTranslationAxis < 0)
            minimumTranslationAxis = -minimumTranslationAxis;
        return std::make_pair(true,
                              minimumTranslationAxis * (minimumTranslation + std::numeric_limits<double>::epsilon()));
    }

  private:
    template <size_t S1, size_t S2>
    constexpr static auto shadowsOverlap(Vector2D<double> axis, std::array<Position<T>, S1> vertices1,
                                         std::array<Position<T>, S2> vertices2) -> std::pair<bool, double>
    {
        ...
        double minimumTranslationVector = right < left ? std::abs(right) : std::abs(-left);
